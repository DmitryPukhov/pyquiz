...
class SubdomainVisitCount:
    ...
    def subdomainVisits(self, cpdomains: List[str]) -> List[str]:
        dictdomains = dict()
        # Build
        self.builddict(cpdomains, dictdomains)
        # DFS
        out = self.visit(dictdomains)
        return out

    def builddict(self, cpdomains: List[str], dictdomains):
        for s in cpdomains:
            n, strdomains = s.split(' ')
            domains = reversed(strdomains.split('.'))
            subdomain = ""
            for domain in domains:
                subdomain = ".".join([domain, subdomain]).lstrip(".")
                visited = dictdomains.get(subdomain, 0)
                visited += int(n)
                dictdomains[subdomain] = visited

    def visit(self, dictdomains) -> List[str]:
        res = list()
        for key in dictdomains.keys():
            res.append("{} {}".format(str(dictdomains[key]), key))
        return res


print(SubdomainVisitCount().subdomainVisits(["9001 discuss.leetcode.com"]))